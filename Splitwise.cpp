...
public:
    ...
    void updateBalance(const string& otherUserId, double amount) {
        ...
        if (std::fabs(balances[otherUserId]) < 0.01) {
...
public:
    ...
    void updateGroupBalance(const string& fromUserId, const string& toUserId, double amount) {
        ...
        if (std::fabs(static_cast<double>(groupBalances[fromUserId][toUserId])) < 0.01) {
            groupBalances[fromUserId].erase(toUserId);
        }
        if (std::fabs(static_cast<double>(groupBalances[toUserId][fromUserId])) < 0.01) {
